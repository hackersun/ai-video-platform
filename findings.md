...
## 2026-07-02 连续动漫制作架构分析发现

- 当前系统已经具备连续动漫生产所需的大部分后端能力：Story Bible/StoryEntity、资产锁、PromptComposer、一致性预检、Production Control、Workflow、TTS、合成、Prompt Skill 和 Seedance 2.0 模型目录均已存在。
- 最大体验问题不是“没有功能”，而是入口和心智割裂：小说、实体、资产、分镜、镜头、视频、TTS、合成、AI 制片、工作流都像独立工具，用户难以把它们理解为一条多集生产线。
- `frontend/src/lib/episode-preview-production.ts` 已经串起本集草片主链路：工作流 -> 剧本 -> 分镜 -> 制片检查 -> 资产合约 -> 批量音视频/字幕 -> 连续成片 -> 渲染预检 -> 渲染包。它应成为 P0 产品化基座。
- Story Bible 应升级为 Production Bible：同一处维护风格、角色视觉、声线、场景、道具状态、事件时间线、Prompt Skill 版本和模型策略；每个 Workflow/Job 保存开拍快照，避免后续设定变化污染历史镜头。
- Seedance-2.0-fast 更适合作为批量草稿模式，Seedance-2.0 更适合作为终稿质量模式；用户层应选择“快速草稿/高质量终稿”等策略，而不是在模型配置页理解所有模型。
- 对标 Runway/Kling/Luma/Sora/Veo，趋势是参考资产、关键帧、多轮编辑、原生音频和角色资产复用；项目现有 asset_version_locks、production_context、generation_preflight 可作为统一适配协议，不宜为每个供应商重写业务流程。
