...
## 2026-06-06 Phase 260 P1 直生音视频生成前一致性预检

- [x] `/video-generation` 直生音视频路径在提交 `/media/generate` 前调用统一 `/consistency/preflight`。
- [x] 预检使用 `task_type=direct_audio_video`，并携带外部适配配置、参考图和完整小说/章节/剧本/分镜/镜头链路。
...
- [x] `/producer` 点击最近批量任务时加载 `/batch/{job_id}/items`，不再只展示总进度。
- [x] 进度卡展示每个批量 item 的镜头、状态、产物任务、产物地址和失败原因。
...
## 2026-06-10 Phase 273 RC 收口：资产抽取与视频参考图边界

- [x] 将实体/资产抽取纳入 Prompt 技能全局管理：新增 `entity_extraction` 标准技能、变量说明、前端任务选择和 AI 入口模板套用测试。
- [x] 收紧实体抽取过滤：组合名会清理为角色本名，群体/情绪/长动作短语不再进入角色资产，道具动作前缀会剥离，明显剧情动作会归为事件。
- [x] 修复镜头生产上下文 JSON 持久化：`production_context` 更新改为复制后重新赋值，避免多视图、资产锁、审核状态只在响应里可见而视频生成读不到。
- [x] 固定视频参考图策略：当前火山视频接口只向供应商提交一个主参考图，多视图和资产锁进入提示词/metadata 作为一致性约束；任务记录显式写入 `provider_reference_image_limit=1` 和补充参考数量。
- [x] 运行后端组合回归：Prompt 技能、实体抽取、Studio、视频参考图和 workflow 关键套件。
- [x] 运行前端组合回归：构建、Prompt 技能 E2E 和 Studio 关键 E2E。
- [x] 提交候选稳定点，作为后续回退锚点。

## Phase 273 成功标准

- 实体抽取的激活模板优先级明确：有激活模板时先走模板包裹后的提示词，没有模板时继续走内部确定性逻辑。
- 抽取结果能减少角色/道具错配：不把群体、情绪词、身体状态、动作短语当角色资产；道具名尽量保留可见物件本体。
- 镜头生产上下文能真实持久化，分镜/镜头加载后不丢多视图、资产锁和审核信息。
- 视频生成不假装支持多实体图片直传；生产模式用预检阻断或提示公网参考图问题，测试模式可通过明确提示/确认继续跑通。
## 2026-07-02 连续动漫制作门槛简化架构分析

### 目标
- 提交近期初版 checkpoint。
- 深入分析现有系统整体架构、业务功能流程和割裂点。
- 输出完整优化方案：降低完整小说、多集连续动漫制作门槛，保证风格、人物、场景、道具、事件、配角、声音一致性。
- 结合 Seedance-2.0 等模型能力、提示词模板和对标平台经验，给出可落地、可验证的实施路线。

### 当前进度
- [x] 完成 checkpoint commit：a9314f1 chore: checkpoint anime production workflow draft
- [x] 代码架构梳理
- [x] 外部模型/平台资料校准
- [x] 现状问题与目标流程设计
