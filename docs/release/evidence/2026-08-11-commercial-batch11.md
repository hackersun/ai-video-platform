...
GitHub 远端 CI 已明确提示 `actions/checkout@v4`、`actions/setup-node@v4`、`actions/setup-python@v5` 和 `actions/upload-artifact@v4` 仍面向已弃用的 Node 20 Actions 运行时。根因不是应用使用的 Node.js 版本，而是工作流引用的 Action 主版本过旧。

本批次只把上述四个官方 Action 统一升级到 v7，并增加合同测试阻止旧主版本回流。没有修改任务名称、触发条件、权限、依赖关系、测试命令、产物名称、分支保护上下文或应用代码。
...
结果：`1 failed`。测试准确识别到 checkout 同时存在 v4/v5，setup-python 为 v5，setup-node 和 upload-artifact 为 v4。
...
- [ ] 远端八项 CI 通过，且没有 Node 20 Actions 运行时弃用提示。
