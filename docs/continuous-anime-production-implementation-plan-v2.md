...
- `video_reference_adapter.py` 已按模型能力矩阵提交 `reference_image` / `reference_video` / `reference_audio` 三类参考内容，并在任务 metadata 中记录图片、视频、音频计数；构建器在模型允许音频参考时会装配 `shot.audio_url` 与锁定音频资产；无可公网提交音频时为 `audios: []`。
- Seedance 2.0 / 2.0 fast 按能力矩阵允许多图多视频；Seedance 1.x、未知模型和 Agent Plan 通道保持 `images:1` 的单图兼容路径。
- 参考包 metadata 已落到 `VideoJob.extra_data.reference_package`，审阅/历史页面可展示 `image_count`、`video_count` 与 `dropped`；`/studio` snapshot 已返回精简参考包证据，生产看板展示最近任务的图片/视频/裁剪计数。
...
- `/studio/cards` 一键补齐入口已支持配置 `min_occurrences`、`image_model_config_id` 与逗号分隔 `voice_pool`；后端会按阈值筛选候选、轮询显式声线池，并把本次使用的图像模型配置写入生成资产 metadata。
