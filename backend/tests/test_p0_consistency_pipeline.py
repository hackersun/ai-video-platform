...
def test_non_dev_tts_generation_persists_preflight_summary_on_provider_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("DEV_MODE", "false")
    user_id = str(uuid4())
    ...
    async def _failing_tts(self, **kwargs):
        raise Exception("upstream failure")

    monkeypatch.setattr("app.services.minimax_service.MiniMaxService.text_to_speech", _failing_tts)
    ...
    async def _load_failed_job() -> TTSJob:
        async with AsyncSessionLocal() as db:
            result = await db.execute(select(TTSJob).where(TTSJob.user_id == user_id))
            return result.scalar_one()

    job = asyncio.run(_load_failed_job())
    assert job.status == "failed"
    assert job.error_message == "upstream failure"
    preflight = (job.extra_data or {}).get("generation_preflight")
    assert preflight["ready"] is True
    assert preflight["blocking_issue_count"] == 0
    assert preflight["issues"] == []
