...
async def list_my_novels(
    ...
    """获取我的小说列表"""
    novels, total = novel_service.get_novels(
        db, skip=skip, limit=limit, author_id=UUID(user_id), status=status
    )
...
async def publish_novel(
    ...
    novel = novel_service.update_novel(
        db, novel_id, NovelUpdate(status="published"), UUID(user_id)
    )
...
async def archive_novel(
    ...
    """归档小说"""
    novel = novel_service.update_novel(
        db, novel_id, NovelUpdate(status="archived"), UUID(user_id)
    )
...
async def create_chapter(
    ...
    if str(novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权为此小说添加章节"
        )
...
async def get_chapter(novel_id: UUID, chapter_id: UUID, db: Session = Depends(get_db)):
    """获取章节详情"""
...
async def update_chapter(
    ...
    chapter = chapter_service.update_chapter(
        db, chapter_id, chapter_data, UUID(user_id)
    )
    return chapter


@router.delete(
    "/{novel_id}/chapters/{chapter_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
@router.post("/generate-cover", response_model=CoverGenerateResponse)
async def generate_cover(
    request: CoverGenerateRequest, user_id: str = Depends(get_current_user_id)
):
    """
    AI生成小说封面
    根据标题、描述、类型生成匹配的封面图片
    ...
    if cover_url:
        return CoverGenerateResponse(
            cover_url=cover_url, success=True, message="封面生成成功"
        )
    else:
        return CoverGenerateResponse(
            cover_url=None, success=False, message="封面生成失败，请稍后重试"
        )
