...
async def list_scripts(
    ...
    scripts, total = await script_service.get_scripts(
        db,
        skip=skip,
        limit=limit,
        novel_id=novel_id,
...
async def get_script(
    ...
    if script.novel and str(script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权查看此剧本"
        )
...
async def update_script(
    ...
    if script.novel and str(script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权修改此剧本"
        )

    script = await script_service.update_script(db, script_id, script_data)
    return script
...
async def delete_script(
    ...
    if script.novel and str(script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权删除此剧本"
        )
...
async def create_scene(
    ...
    if script.novel and str(script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权为此剧本添加场景"
        )
...
async def get_scene(
    script_id: UUID, scene_id: UUID, db: AsyncSession = Depends(get_db)
):
    """获取场景详情"""
...
async def update_scene(
    ...
    if script.novel and str(script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权修改此场景"
        )
...
async def delete_scene(
    ...
    if script.novel and str(script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权删除此场景"
        )
...
async def generate_script(
    ...
    if str(novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权为此小说生成剧本"
        )

    # TODO: 提交Celery任务进行AI生成
    # script = await script_service.generate_script_from_chapter(
    #     db,
...
async def generate_scene_video(
    ...
    if scene.script.novel and str(scene.script.novel.author_id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="无权为此场景生成视频"
        )
...
async def get_generation_status(
    ...
    return GenerationResult(
        task_id=task_id, status="processing", progress=50, created_at=datetime.utcnow()
    )
