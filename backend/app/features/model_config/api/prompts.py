...
async def create_prompt_profile(request: CreateResourceRequest):
    del request
    return unsupported("prompt_profile.create")


@router.post("/prompt-profiles/{profile_id}/versions")
async def create_prompt_profile_version(profile_id: str, request: CreateVersionRequest):
    del profile_id, request
    return unsupported("prompt_profile_version.create")


@router.post("/prompt-profile-versions/{version_id}/publish")
async def publish_prompt_profile_version(version_id: str, request: PublishRequest):
    del version_id, request
    return unsupported("prompt_profile_version.publish")
...
async def rollback_prompt_profile(profile_id: str, request: RollbackRequest):
    del profile_id, request
    return unsupported("prompt_profile.rollback")
