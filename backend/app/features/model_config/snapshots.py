...
_FORBIDDEN_KEY_PARTS = frozenset({"api_key", "api_secret", "authorization", "prompt", "text", "token", "secret"})
SNAPSHOT_PARAM_ALLOWLIST = frozenset({
    ...
    "speed", "parameter_normalization", "provider_prompt_sanitized", "reference_image_strategy",
})
...
def _forbidden_keys(value: Any) -> set[str]:
    ...
    found = {
        ...
        if any(part in str(key).strip().lower() for part in _FORBIDDEN_KEY_PARTS)
    }
...
def sanitize_snapshot_params(params: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    sanitized = {key: values[key] for key in sorted(SNAPSHOT_PARAM_ALLOWLIST & values.keys())}
    try:
