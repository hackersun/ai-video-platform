...
test('producer keeps one-click generation evidence with model status and produced ids', async ({ page }) => {
  ...
  expect(generateRequests[0]).toMatchObject({ model_config_id: 'text-config-001' });
  expect(generateRequests[0]).not.toHaveProperty('shot_count');
...
test('producer updates the selected workflow after one-click generation and loads the produced storyboard shots', async ({ page }) => {
  const statusRequests: string[] = [];
  const shotRequests: string[] = [];
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/api/v1/workflow/wf-old/step') {
      workflowUpdates.push(request.postData() ? JSON.parse(request.postData() || '{}') : {});
      await route.fulfill({ status: 200, contentType: 'application/json', body: '{}' });
      return;
    }

    if (path === '/api/v1/workflow/status/wf-old') {
      statusRequests.push('wf-old');
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          ...
          chapter_id: 'chapter-001',
          script_id: null,
          storyboard_id: workflowUpdates.length > 0 ? 'storyboard-new' : null,
          video_jobs: [],
          tts_jobs: [],
          synthesis_jobs: [],
        }),
    ...
    if (path === '/api/v1/workflow/status/wf-created') {
      statusRequests.push('wf-created');
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    if (path === '/api/v1/short-video/workflow/wf-old/readiness' || path === '/api/v1/short-video/workflow/wf-created/readiness') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ summary: { ready: false, score: 50, blocker_count: 0, warning_count: 1, shot_count: 0 } }),
      });
      return;
    }
    ...
    if (path === '/api/v1/chapters/chapter-001/production-status') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ script_id: 'script-001', storyboard_id: 'storyboard-new', shot_count: 1, has_script: true, has_storyboard: true }),
      });
      return;
    }
    ...
    if (path === '/api/v1/shots/storyboard/storyboard-new') {
      shotRequests.push('storyboard-new');
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([{ id: 'shot-new-001', shot_number: 1, title: '雪夜出山', description: '少年推门踏入风雪。' }]),
      });
      return;
    }
  ...
  await expect(page.getByRole('combobox').nth(2)).toHaveValue('wf-old');
  await page.getByRole('button', { name: /^全部$/ }).click();

  await expect(page.getByTestId('producer-one-click-evidence')).toContainText('wf-old');
  await expect(page.getByRole('combobox').nth(2)).toHaveValue('wf-old');
  await expect.poll(() => workflowUpdates.length).toBe(1);
  expect(workflowUpdates[0]).toMatchObject({ storyboard_id: 'storyboard-new', script_id: 'script-001' });

  await page.getByRole('button', { name: /加载镜头/ }).click();

  await expect.poll(() => shotRequests).toEqual(['storyboard-new']);
  await expect(page.getByText('镜头 1').nth(1)).toBeVisible();
  expect(statusRequests).toContain('wf-old');
});

test('producer does not auto-switch away from a newly created workflow before the workflow list refreshes', async ({ page }) => {
  const shotRequests: string[] = [];
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/api/v1/short-video/workflow/wf-created/readiness' || path === '/api/v1/short-video/workflow/wf-old/readiness') {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ summary: { ready: false, score: 50 } }) });
      return;
    }
  ...
  await page.goto('/producer?novel_id=novel-001');
  await page.getByRole('combobox').nth(1).selectOption('chapter-001');
  await page.getByRole('combobox').nth(2).selectOption('');

  await page.getByRole('button', { name: /^全部$/ }).click();

  await expect(page.getByTestId('producer-one-click-evidence')).toContainText('wf-created');
  await expect(page.getByRole('combobox').nth(2)).toHaveValue('wf-created');
