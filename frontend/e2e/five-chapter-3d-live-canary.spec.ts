...
const enabled = process.env.FIVE_CHAPTER_LIVE === '1';
...
async function certifyFromUi(page: any, model: typeof certifiedModels[number]) {
  ...
  await page.getByLabel('操作原因').fill(`五章3D实模前置验证：${model.label}`);
  await page.getByRole('button', { name: '提交认证' }).click();
  await expect(page.getByText('已通过', { exact: true })).toBeVisible({ timeout: 120_000 });
}

test('sunqy creates a fresh five-chapter 3D novel and generates three representative native-audio videos from UI', async ({ page }, testInfo) => {
  test.setTimeout(35 * 60_000);
  test.skip(!enabled, 'explicit five-chapter live authorization required');
  expect(testInfo.retry).toBe(0);
  expect(userId).toBeTruthy();
  expect(Number(process.env.FIVE_CHAPTER_LIVE_MAX_RMB)).toBe(10);
  ...
  await expect(panel.getByTestId('series-run-episodes').getByText('镜头就绪')).toHaveCount(5, { timeout: 240_000 });
  ...
  await page.screenshot({ path: testInfo.outputPath('01-five-chapter-ready.png'), fullPage: true });

  const generationResponse = page.waitForResponse((response) => response.url().endsWith('/generate-selected') && response.request().method() === 'POST');
  ...
  const evidence = {
    title, novel_id: novelId, run_id: runId, selected_shot_ids: selectedShotIds,
