...
export function SeriesRunView({ state, actions }: { state: any; actions: any }) {
  const { run, busy, error, validatedBindings, anchors, anchorBlockers, anchorMode,
    ...
    qualityReports, generationStatus, generationQuality, nativeAudio, preflightBusy, chapters, recovery } = state;
  const { start, act, enableLiveCanary, refreshPreflightPlan, setSelectedVoiceId,
    lockVoice, prepareReference, setStoryLockBusy, chooseMode, saveSelection,
    generateSelected, setNativeAudio, executeRecovery } = actions;
  ...
  return (
    <section className="space-y-4 rounded-xl border border-violet-400/20 bg-violet-500/5 p-4" data-testid="series-run-panel">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><h3 className="font-semibold text-white">整书自动制作</h3><p className="text-sm text-white/50">一次推进四章到镜头就绪，再选择跨章关键镜头验证一致性。</p></div>
        {!run ? <Button onClick={start} disabled={busy || chapters.length !== 4} className="bg-violet-600 hover:bg-violet-700">{busy && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}整书自动制作</Button> : (
      ...
      {chapters.length !== 4 && <div className="text-sm text-amber-200">当前为 {chapters.length} 章；本次标准化验证要求恰好 4 章。</div>}
      ...
      {run && <>
        ...
        <div className="rounded-lg border border-amber-400/20 bg-amber-500/5 p-3">
          {run.budget_policy?.live_canary === true ? <div className="text-sm text-emerald-200">实模关键镜头验证已启用 · 服务端预算上限 ¥{run.budget_policy.max_rmb}</div> : <Button size="sm" variant="outline" onClick={enableLiveCanary} disabled={preflightBusy}>启用实模关键镜头验证</Button>}
        ...
        {preflightPlan && <div className="space-y-3 rounded-lg border border-sky-400/20 bg-sky-500/5 p-3" data-testid="live-preflight-plan">
          ...
          <div className="flex flex-wrap gap-2"><StoryLockControl runId={run.id} disabled={preflightBusy} onPrepared={() => refreshPreflightPlan(run.id, nativeAudio)} onLoading={setStoryLockBusy} /><Button size="sm" variant="outline" onClick={prepareReference} disabled={preflightBusy || preflightPlan.blocker_codes.some((code) => ['story_bible_missing', 'state_machine_missing', 'story_lock_stale', 'trusted_budget_policy_missing', 'projected_budget_exceeded', 'model_bindings_not_fresh', 'production_entities_unapproved', 'production_entity_conflict'].includes(code))}>生成并锁定参考图</Button><Button size="sm" variant="ghost" onClick={() => refreshPreflightPlan(run.id, nativeAudio)} disabled={preflightBusy}>刷新前置状态</Button></div>
        ...
        {anchors.smoke.length > 0 && <div className="space-y-3">
          ...
          <label className="flex items-start gap-3 rounded-lg border border-violet-400/20 bg-violet-500/5 p-3 text-sm text-white/70">
            ...
            <span><span className="block font-medium text-white">Seedance 1.5 原生配音（临时）</span><span className="text-xs text-white/50">仅本次生成有效；开启后把对白、普通话、口型、情绪和禁止改写约束交给视频模型，并跳过独立 TTS。</span></span>
          </label>
          ...
          <AnchorShotSelector recommendations={anchors[anchorMode]} selected={selected} onChange={(ids) => saveSelection(ids)} onGenerate={generateSelected} busy={preflightBusy || Boolean(anchorBlockers[anchorMode]) || !selectedMode} />
        </div>}
