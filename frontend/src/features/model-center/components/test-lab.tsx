...
function LiveConfirmation({ canSubmit, onClose, onSubmit }: { canSubmit: boolean; onClose: () => void; onSubmit: () => void }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label="真实费用确认" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/75 p-4"><section className="w-full max-w-lg rounded-xl border border-amber-300/25 bg-slate-900 p-5 shadow-2xl"><h2 className="text-lg font-semibold text-white">真实费用确认</h2><p className="mt-2 text-sm leading-6 text-slate-300">真实验证会按已确认的预算、镜头和策略提交安全执行意图；运行结束后需回到本页查看脱敏证据。</p><label className="mt-5 flex items-start gap-2 text-sm text-amber-100"><input aria-label="本次会产生真实费用" type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />本次会产生真实费用</label>{!canSubmit && <p className="mt-3 text-xs text-amber-100">请先完整填写模型、连接、运行上下文、镜头、预算、重试与存储策略。</p>}<div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="model-center-quiet">取消</button><button type="button" disabled={!confirmed || !canSubmit} onClick={onSubmit} className="model-center-primary">提交真实验证</button></div></section></div>;
}
...
export function TestLab({ runId, location }: { runId?: string; location: ModelCenterLocation }) {
  ...
  const submitLive = async () => {
    ...
    try {
      const created = await run.createCertification({ profile_version_id: profileVersionId, connection_id: connectionId, level: 'live', reason, user_scope: liveContext.userScope.trim(), recipe_version_id: liveContext.recipeVersion.trim(), chapter_id: liveContext.chapterId.trim(), run_id: liveContext.runId.trim(), selected_shot_ids: liveContext.selectedShots.split(',').map((item) => item.trim()).filter(Boolean), budget_ceiling_rmb: liveContext.budgetCeiling.trim(), retry_policy: liveContext.retryPolicy, storage_policy: liveContext.storagePolicy, real_cost_acknowledged: true });
      router.push(modelCenterHref({ ...location, section: 'test-lab', runId: created.id }));
    } catch (error) {
