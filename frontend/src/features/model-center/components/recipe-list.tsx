...
export function RecipeList() {
  ...
  return <div className="p-4">
    ...
    {rows.length ? <div className="mt-4 overflow-x-auto rounded-lg border border-white/10"><table className="min-w-full text-left text-sm"><thead className="bg-white/[0.035] text-xs text-slate-500"><tr><th>名称</th><th>策略</th><th>版本</th><th>状态</th><th>操作</th></tr></thead><tbody>{rows.map((recipe) => <tr key={recipe.id} className="border-t border-white/[0.07] text-slate-300"><td className="font-medium text-white">{recipe.name}</td><td>{recipe.strategy || '未声明'}</td><td>v{recipe.version}</td><td>{statusLabel[recipe.status]}</td><td><button type="button" aria-label={`查看${recipe.name}`} onClick={() => setSelectedId(recipe.id)} className="model-center-quiet"><Eye className="h-3.5 w-3.5" />查看</button></td></tr>)}</tbody></table></div> : <ModelCenterEmpty title="还没有生产方案" description="新建方案后，按发布版本供工作台选择。" />}
