...
export function PromptUsageSummary({
  ...
  const notApplicable = summary.counts.not_applicable || 0;
  const promptStages = summary.total - notApplicable;
  return <div data-testid="prompt-usage-summary" className="grid gap-3 border-b border-white/10 p-4 sm:grid-cols-3">
    <div className="rounded-xl border border-violet-400/20 bg-violet-500/[0.07] p-4">
      <div className="flex items-center gap-2 text-xs text-violet-300"><Layers3 className="h-4 w-4" />使用模板环节</div>
      <p className="mt-2 text-2xl font-semibold text-white">{promptStages} / {summary.total}</p>
      <p className="mt-1 text-xs text-slate-400">另有 {notApplicable} 个环节无需模板</p>
    </div>
    <div className="rounded-xl border border-emerald-400/20 bg-emerald-500/[0.06] p-4">
      ...
      <p className="mt-2 text-2xl font-semibold text-white">{healthy} / {promptStages}</p>
    </div>
    <button type="button" onClick={onToggleProblems} className={`rounded-xl border p-4 text-left transition-colors ${problemsOnly ? 'border-amber-400/60 bg-amber-500/15' : 'border-amber-400/20 bg-amber-500/[0.06] hover:bg-amber-500/10'}`}>
      <div className="flex items-center justify-between gap-2 text-xs text-amber-300"><span className="flex items-center gap-2"><AlertTriangle className="h-4 w-4" />提示词问题</span><span>{problemsOnly ? '查看全部环节' : '只看问题环节'}</span></div>
      <p className="mt-2 text-2xl font-semibold text-white">{issues} <span className="text-sm font-normal text-slate-400">个</span></p>
    </button>
  </div>;
}
