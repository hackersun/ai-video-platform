...
export function ModelCenterCatalogPanel({ location }: { location: ModelCenterLocation }) {
  ...
  const overview = useModelCenterOverview();
  ...
  return (
    <div className="p-4">
      <div className="mb-4 flex items-center justify-between gap-3"><p className="text-xs text-slate-500">系统模型目录对所有用户可见；API Key 和默认模型仅保存在当前账号。</p>{overview.data?.can_manage_catalog && <button type="button" onClick={() => setCreating(true)} className="model-center-primary"><Plus className="h-4 w-4" />新增模型</button>}</div>
