...
export function ModelCenterCatalogPanel({ location }: { location: ModelCenterLocation }) {
  ...
  return (
    <div className="p-4">
      ...
      {!models.length ? <ModelCenterEmpty title="没有匹配的模型版本" description="请先添加模型，或切换查看全部用途。" /> : <div className="overflow-x-auto rounded-lg border border-white/10"><table className="min-w-full text-left text-sm"><thead className="bg-white/[0.035] text-xs text-slate-500"><tr><th>模型名称</th><th>供应商</th><th>用途</th><th>当前默认</th><th>配置版本</th><th>验证</th><th className="text-right">操作</th></tr></thead><tbody>{models.map((model) => { const defaultTasks = model.profile_version_id ? defaultsByVersion.get(model.profile_version_id) || [] : []; return <tr key={`${model.profile_version_id || model.legacy_model_id || model.api_model_id}-${model.provider_id}`} className="border-t border-white/[0.07] text-slate-300"><td><p className="font-medium text-white">{model.model_name}</p><p className="mt-0.5 text-[11px] text-slate-500">{model.api_model_id}</p></td><td><ProviderModelLabel providerName={model.provider_name} providerCode={model.provider_code} /></td><td><div className="flex flex-wrap gap-1">{model.capabilities.map((item) => <span key={item} className="rounded border border-violet-400/25 bg-violet-500/10 px-1.5 py-0.5 text-[11px] text-violet-200">{labels[item]}</span>)}</div></td><td>{defaultTasks.length ? <span className="inline-flex items-center gap-1 text-xs text-amber-200"><Star className="h-3.5 w-3.5 fill-current" />{defaultTasks.join('、')}</span> : <span className="text-xs text-slate-600">未设为默认</span>}</td><td><span className="block">{model.profile_version ? `v${model.profile_version}` : '兼容配置'}</span><span className="text-[11px] text-slate-500">{model.driver_key || '旧版适配'}</span></td><td><span className={model.certification_status === 'unverified' ? 'text-amber-200' : 'inline-flex items-center gap-1 text-emerald-300'}>{model.certification_status !== 'unverified' && <CheckCircle2 className="h-3.5 w-3.5" />}{certificationLabel(model.certification_status)}</span></td><td className="text-right"><button type="button" aria-label={`查看 ${model.model_name}`} onClick={() => setSelected(model)} className="model-center-quiet"><PlayCircle className="h-3.5 w-3.5" />查看与设置</button></td></tr>; })}</tbody></table></div>}
