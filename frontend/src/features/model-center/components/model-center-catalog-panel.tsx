...
export function ModelCenterCatalogPanel({ capability }: { capability?: ModelCapability }) {
  ...
  const models = (data?.items || []).filter((item) => !capability || item.capabilities.includes(capability));
  return (
    <div className="p-4">
      {!models.length ? <ModelCenterEmpty title="没有匹配的模型版本" description="请先在连接中保存凭证，或切换查看全部能力。" /> : <div className="overflow-x-auto rounded-lg border border-white/10"><table className="min-w-full text-left text-sm"><thead className="bg-white/[0.035] text-xs text-slate-500"><tr><th>模型名称</th><th>提供方</th><th>能力</th><th>配置版本</th><th>认证</th><th className="text-right">操作</th></tr></thead><tbody>{models.map((model) => <tr key={`${model.profile_version_id || model.legacy_model_id || model.api_model_id}-${model.provider_id}`} className="border-t border-white/[0.07] text-slate-300"><td><p className="font-medium text-white">{model.api_model_id}</p><p className="mt-0.5 text-[11px] text-slate-500">驱动与契约由服务端目录解析</p></td><td>{model.provider_id}</td><td><div className="flex flex-wrap gap-1">{model.capabilities.map((item) => <span key={item} className="rounded border border-violet-400/25 bg-violet-500/10 px-1.5 py-0.5 text-[11px] text-violet-200">{labels[item]}</span>)}</div></td><td>{model.profile_version_id || '兼容配置'}</td><td><span className={model.certification_status === 'none' ? 'text-amber-200' : 'inline-flex items-center gap-1 text-emerald-300'}>{model.certification_status !== 'none' && <CheckCircle2 className="h-3.5 w-3.5" />}{model.certification_status === 'none' ? '待认证' : model.certification_status}</span></td><td className="text-right"><span className="inline-flex items-center gap-1 text-xs text-slate-500"><PlayCircle className="h-3.5 w-3.5" />查看</span></td></tr>)}</tbody></table></div>}
