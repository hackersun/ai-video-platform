...
const headingBySection = {
  ...
  connections: ['供应商账号', '保存 API 凭证；同一供应商可按套餐、环境或账号分别管理。'],
  catalog: ['模型目录', '按能力查看已接入模型版本和认证等级。'],
  bindings: ['默认模型', '选择文本、图片、视频和配音任务实际使用的模型。'],
  recipes: ['组合预设', '将文本、图像、视频、语音的生产组合版本化。'],
  prompts: ['提示词使用地图', '查看每个生产环节当前实际使用的模板、模型和覆盖来源。'],
...
export function ModelCenterShell() {
  ...
  const content = location.section === 'overview' ? <ModelCenterOverviewPanel location={location} />
    : location.section === 'connections' ? <ModelCenterConnectionsPanel location={location} />
      : location.section === 'catalog' ? <ModelCenterCatalogPanel location={location} />
        : <ModelCenterManagementPanel section={location.section} location={location} />;

  const showCapabilityFilters = location.section !== 'prompts';
  return <div className={`model-center-shell ${location.section === 'prompts' ? 'model-center-shell-wide' : ''} -mx-4 -mt-8 min-h-[calc(100vh-4rem)] border-y border-white/10 sm:-mx-8`}><ModelCenterRail activeSection={location.section} location={location} /><main className="min-w-0 p-5 lg:p-6"><ModelCenterCompactNav activeSection={location.section} location={location} /><header className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h1 className="text-2xl font-bold tracking-tight text-white">{title}</h1><p className="mt-1 text-sm text-slate-400">{description}</p></div><div className="flex items-center gap-2"><Link href={currentHref} className="model-center-quiet"><RefreshCw className="h-4 w-4" />刷新</Link>{location.section === 'connections' && <a href="#connection-form" className="model-center-primary"><Plus className="h-4 w-4" />新增账号</a>}</div></header>{showCapabilityFilters && <div className="mb-4 flex flex-wrap items-center gap-2 border-b border-white/10 pb-4"><Link href={modelCenterSectionHref('catalog', { ...location, capability: undefined })} className={`model-center-filter ${!location.capability ? 'model-center-filter-active' : ''}`}>全部模型</Link>{[['文本', 'text_generation'], ['图像', 'image_generation'], ['视频', 'video_generation'], ['语音', 'speech_generation']].map(([label, capability]) => <Link key={capability} href={modelCenterSectionHref('catalog', { ...location, capability: capability as never })} className={`model-center-filter ${location.capability === capability ? 'model-center-filter-active' : ''}`}>{label}</Link>)}<span className="ml-auto hidden items-center gap-1 text-xs text-slate-500 lg:inline-flex"><SlidersHorizontal className="h-3.5 w-3.5" />配置保存于服务端</span></div>}<section className="overflow-hidden rounded-xl border border-white/10 bg-slate-950/25 shadow-[0_16px_48px_rgba(0,0,0,0.12)]">{content}</section></main>{location.section !== 'prompts' && <ModelCenterInspector section={location.section} location={location} />}</div>;
}
